...
const PRIMARY_SPLINE_SCENE = "https://prod.spline.design/Ap3r1jgs0kwQjDC/scene.splinecode";
...
class SplineErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.warn("Spline 3D error caught by boundary:", error, errorInfo);
  }
...
interface SplineScene3DProps {
  ...
  showWatermarkCover?: boolean;
}

export default function SplineScene3D({
  sceneUrl = PRIMARY_SPLINE_SCENE,
  className = "",
  isInteractive = true,
  opacity = 1,
  onLoad,
  showWatermarkCover = false,
}: SplineScene3DProps) {
  ...
  const handleSplineError = () => {
    if (currentUrl === PRIMARY_SPLINE_SCENE) {
  ...
  const fallbackMarkup = (
    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#050814]/80 via-[#0B0F19]/60 to-[#050814]/90">
      <div className="w-full h-full relative overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full bg-indigo-600/10 blur-[100px] animate-pulse" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full bg-cyan-500/10 blur-[100px] animate-pulse" />
      </div>
    </div>
  );
  ...
  return (
    <div
      className={`relative w-full h-full overflow-hidden transition-opacity duration-700 select-none ${className}`}
      style={{ opacity }}
    >
      {/* 3D Loading Fallback */}
      {!isLoaded && !hasError && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-transparent backdrop-blur-xs select-none pointer-events-none">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/40 border border-white/10 backdrop-blur-md text-white/70 text-[10px] font-mono">
            ...
            <span>Initializing 3D Orbit...</span>
          </div>
        </div>
      )}

      {/* Real Spline 3D Scene with Error Boundary */}
      {!hasError ? (
      ...
      {/* Subtle bottom-right glass cover for clean presentation */}
      {showWatermarkCover && (
        <div className="absolute bottom-2 right-2 z-10 pointer-events-none px-2 py-0.5 rounded-md bg-background/40 backdrop-blur-md border border-border/40 text-[9px] font-mono text-muted-foreground/70 opacity-60">
          3D Engine Active
        </div>
      )}
    </div>
